...
const result: Record<string, RecipeResult> = {};
...
for (const fileName of fs.readdirSync(bedrockRecipesPath)) {
  ...
  result[inputOverride ?? recipe.input] =
    typeof recipe.output === "string"
      ...
      : {
          item: outputOverride ?? recipe.output.item,
...
fs.writeFileSync(
  path.join(scriptsGeneratedDirPath, "powered_furnace_recipes.ts"),
  `export const POWERED_FURNACE_RECIPES:Record<string, {item: string, count: number}>=${JSON.stringify(result)};`,
);
